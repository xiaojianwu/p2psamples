					   ...
					   const pj_str_t *local_passwd)
{
	...
	for (j=0; j<comp->cand_cnt; ++j) {
	    ...
	    /* Add the candidate */
		//使用pj_ice_sess_cand结构体来表示候选者实例，使用pj_ice_sess_add_cand()为每一个构建添加本地地址候选. 
		//每个构件初始化过程中必须至少有一个候选地址，并且所有的控件必须拥有相同的候选地址数，不然会导致配对过程的失败。
					     ...
					     const pj_ice_sess_cand rem_cand[])
{
    ...
    /* Build check list */
	//当本地接收到对端的应答（包含了对端的候选者信息），应马上解析该消息，建立关于对端的候选者列表，
	//并且通过pj_ice_sess_create_check_list()来建立check lists，这个过程就称为配对.
